...
uint32_t Get_Hex_Format(FILE *fp)
{
    ...
    Put_Error(stderr, result, 1, line);
    return result;
}
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        while(getline((char**)&pline, &len, fp_src) != -1)
        {
            ...
            if (pass) // pass1
            {
                ...
                error = error + Put_Error(stderr, result, pass, line);
            }
            else // pass0
            {
...
void Exec_Instruction(void)
{
    ...
    if (i < instr_max)
    {
        switch(INSTR[i].index)
        {
            ...
            default : {fprintf(stderr, "ERROR: Internal Bug.\n"); break;}
        }
    }
    // Illegal Instruction
    else
    ...
    while(1)
    {
        ...
        if (scanf(" %s",cmd ))
        {
            ...
            switch(*pcmd++)
            {
                ...
                default :
                {
                    fprintf(stderr, "ERROR: Illegal Simulation Command.\n");
                    break;
                }
            }
        }
